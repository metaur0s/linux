...
#define           brot64 brot64_l
static inline u64 brot64_l (const u64 x, const uint q) { return (x << q) | (x >> (64 - q)); }
static inline u64 brot64_r (const u64 x, const uint q) { return (x >> q) | (x << (64 - q)); }
...
#define _USES_OFFSET(x) ((x) - SKB_HEAD(skb))
#else
#define _USES_OFFSET(x)  (x)
#endif
