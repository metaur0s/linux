...
#define KEYS_LOAD(r) \
    u64 A = (K[0] * r) + K[1], B = (K[2] * r) + K[3], \
        C = (K[4] * r) + K[5], D = (K[6] * r) + K[7]

//
#define KEYS_ENC(x) (  swap64(  swap64(  swap64((x) + A) + C) + B) + D)
#define KEYS_DEC(x) (unswap64(unswap64(unswap64((x) - D) - B) - C) - A)

// AVALANCHE OF ORIGINAL THROUGH KEYS
#define KEYS_ROTATE(x) A += B += C += D += ((A + D) * (C + B)) + x

// RETURN THE HASH
#define KEYS_HASH ((A * D) + C) ^ B
