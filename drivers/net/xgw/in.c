...
#define PR_CONNECTING  ((u64)1024) // TEM QUE SER GRANDE SUFICIENTE PARA SER MAIOR DO QUE QUALQUER RTT - OBSERVAR A LOGICA NO KEEPER ACKS
#define PR_ESTABLISHED ((u64)2048)

#define XTIME_MIN ((u64)(8192ULL))
...
static noinline uint in_ping (node_s* const node, const skb_s* const skb, pkt_s* const pkt) {
    ...
    ASSERT(i >= I_KEY_PING);
    ASSERT(i <= I_KEY_SYN);
    ASSERT(ltime >= XTIME_MIN || i == I_KEY_SYN);
    ASSERT(ltime <= XTIME_MAX || i == I_KEY_SYN);
    ...
    if (tdiff)
        ...
        if (ABS_DIFF((rtime + lag), RTIME(now, tdiff)) > (1024 + path->iskew))
